...
import { Users, FileText, Edit, Trash2, X, ShieldCheck, Search, Eye, Coins } from "lucide-react";
  ...
  const salvarEdicaoUsuario = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("usuarios").update({
        ...
        creditos: parseInt(usuarioEditando.creditos || 0, 10) // Salvando os créditos
      ...
      alert("Usuário e Créditos atualizados com sucesso.");
    } catch (err: any) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1">Gestão global de corretores, permissões, laudos e saldo de créditos.</p>
          </div>
          <Link href="/" className="bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 px-5 py-2.5 rounded-xl text-sm font-bold transition-all shadow-sm flex items-center gap-2">
            Voltar para a Home
          </Link>
        </div>
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {usuarios.map((user) => (
                  <tr key={user.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    {/* COLUNA DE CRÉDITOS */}
                    <td className="p-4">
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${user.creditos > 0 ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
                        <Coins size={14} />
                        {user.creditos || 0}
                      </span>
                    </td>
                    ...
                    <td className="p-4 text-right pr-6">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button onClick={() => abrirModalEdicao(user)} className="p-2 bg-slate-100 text-slate-600 hover:bg-slate-200 rounded-lg transition-colors" title="Editar Usuário e Créditos">
                          <Edit size={18} />
                        </button>
        ...
        {/* ========================================== */}
        {/* MODAL 1: EDIÇÃO DE USUÁRIO (COM CRÉDITOS) */}
        {/* ========================================== */}
            ...
            <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
              ...
              <form onSubmit={salvarEdicaoUsuario} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-100">
                  ...
                  {/* CAMPO DE CRÉDITOS */}
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-1">Saldo de Créditos</label>
                    <div className="relative">
                      ...
                      <input 
                        type="number" 
                        min="0"
                        value={usuarioEditando.creditos || 0} 
                        onChange={(e) => setUsuarioEditando({...usuarioEditando, creditos: e.target.value})} 
                        className="w-full pl-10 p-3 border border-slate-300 rounded-lg focus:ring-2 outline-none font-bold text-slate-800" 
                      />
                    </div>
                  </div>
                </div>

                <button type="submit" className="w-full py-3 mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg transition-colors">
                  Salvar Alterações
                </button>
              </form>
            </div>
        ...
        {/* ========================================== */}
        {/* MODAL 2: VER LAUDOS DO USUÁRIO (MANTIDO)   */}
        {/* ========================================== */}
        {modalLaudosOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
